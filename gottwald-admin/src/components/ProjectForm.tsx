...
export default function ProjectForm({ mode, initialData }: ProjectFormProps) {
  ...
  const onSubmit = async (data: ProjectFormValues) => {
    ...
    try {
      ...
      // Step 2: Build final project object
      ...
      if (mode === "create") {
        ...
        await updateProject(data.slug, projectData);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-sm font-medium text-zinc-300 mb-1.5 block">Slug</label>
          <input
            {...register("slug")}
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`text-xs ${(watch("description")?.length || 0) > 120 ? "text-red-400" : "text-zinc-500"}`}>
      ...
      {/* Details */}
      <div>
        <label className="text-sm font-medium text-zinc-300 mb-1.5 block">Details</label>
        <textarea
          {...register("details")}
      ...
      {/* Cover Image */}
      <div>
        <label className="text-sm font-medium text-zinc-300 mb-1.5 block">Cover Image</label>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
        ...
        <div className="flex items-start gap-4">
          {imagePreview ? (
            <div className="relative w-40 aspect-video rounded-lg overflow-hidden bg-zinc-800 shrink-0">
              ...
              <button
                type="button"
                onClick={() => { setImageFile(null); setImagePreview(""); setValue("image", ""); }}
      ...
      {/* Launch URL */}
      <div>
        <label className="text-sm font-medium text-zinc-300 mb-1.5 block">Launch URL</label>
        <input
          {...register("launchUrl")}
          placeholder="https://example.com"
          className="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500 transition-colors"
        />
        {errors.launchUrl && <p className="text-xs text-red-400 mt-1">{errors.launchUrl.message}</p>}
      </div>
      ...
      {/* Services */}
      <div>
        <label className="text-sm font-medium text-zinc-300 mb-1.5 block">Services</label>
      ...
      {/* Theme Colors */}
      <div>
        <label className="text-sm font-medium text-zinc-300 mb-3 block">Theme Colors</label>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {(["background", "text", "accent"] as const).map((key) => (
            <div key={key} className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="text-xs font-medium text-zinc-300 capitalize">{key}</p>
                <p className="text-[10px] font-mono text-zinc-500">{theme[key]}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        >
          ...
          <p className="text-xs mt-1 opacity-70">This is how the project page will look.</p>
          <span
